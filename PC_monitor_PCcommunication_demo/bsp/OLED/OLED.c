...
static I2C_HandleTypeDef *g_pHI2COLED = &hi2c1;//根据配置的I2C句柄进行更改(仅硬件IIC使用)
#else
/**
  * 函    数：OLED写SCL高低电平
  ...
  */
void OLED_W_SCL(GPIO_PinState PinState)
{
	/*根据PinState的值，将SCL置高电平或者低电平*/
	HAL_GPIO_WritePin(OLED_SCL_GPIO_Port, OLED_SCL_Pin,PinState);
...
/**
  ...
  */
void OLED_W_SDA(GPIO_PinState PinState)
{
	/*根据PinState的值，将SDA置高电平或者低电平*/
	HAL_GPIO_WritePin(OLED_SDA_GPIO_Port, OLED_SDA_Pin,PinState);
...
{
	#if OLED_USE_HARDWARE_I2C
	uint8_t tmpbuf[2];	//数据缓冲区
	tmpbuf[0] = 0;       // 控制字节
	tmpbuf[1] = Command; // 命令数据字节
...
{
	#if OLED_USE_HARDWARE_I2C
	HAL_I2C_Mem_Write(g_pHI2COLED, OELD_I2C_ADDR, 0x40, 1, Data, Count, OLED_TIMEOUT);
	#else
	uint8_t i;
...
void OLED_Init(void)
{
	//OLED_GPIO_Init();			//先调用底层的端口初始化（仅标准库使用）
...
/*********************功能函数*/
//直接操作OLED显示Buf数组接口
void OLED_Edit_DisplayBuf(uint8_t page,uint8_t data,uint8_t data_input)
{
	 OLED_DisplayBuf[page][data]=data_input;
}
